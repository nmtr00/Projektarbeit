...
parameter_pattern = re.compile(r'\s*c(\d+)\s*=\s*\[\s*(\d+\.\d+)\s*,\s*(\d+\.\d+)\s*\]')
...
for line in log_content:
    ...
    if match:
        ...
        parameter_values[param] = (value1, value2)
count1 = 0 
count2 = 0
for param, (value1, value2) in parameter_values.items():
    ...
    for old_file_path_1 in files1:
        ...
        new_file_name_1 = old_file_name_1.replace(f'c{param}_force.csv', f'[{value1}, {value2}]_force.csv')
...
for param, (value1, value2) in parameter_values.items():
    ...
    for old_file_path_2 in files2:
        ...
        new_file_name_2 = old_file_name_2.replace(f'c{param}_area.csv', f'[{value1}, {value2}]_area.csv')
